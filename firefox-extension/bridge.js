 ...
 *
 * widget-creator.js runs in the page's MAIN world so it can reach Discord's
 * webpack internals, but MAIN-world scripts have no access to chrome.runtime.
 * This bridge runs in the normal (isolated) content-script world, which CAN
...
(function () {
  ...
  // Firefox uses a promise-based `browser.*` API; Chrome uses a callback-based
  // `chrome.*` API. Pick the right calling convention for each so the response
  // actually comes back (a callback passed to browser.* is ignored / errors).
